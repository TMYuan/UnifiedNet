...
class KTH(object):
    ...
    def __init__(self, train, data_root, seq_len = 20, image_size=64, data_type='drnet', transforms=None):
    ...
    def get_drnet_data(self):
        ...
        #     seq.append(im)
        t = np.random.randint(seq_len - 5)
        t_2 = np.random.randint(5)
        fname_1 = '%s/%s' % (dname, vid['files'][seq_idx][t])
        fname_2 = '%s/%s' % (dname, vid['files'][seq_idx][t + t_2])
    ...
    def __getitem__(self, index):
        ...
        if self.data_type == 'sequence':
            ...
            img_1, img_2 = self.get_drnet_data()
            if self.transforms:
