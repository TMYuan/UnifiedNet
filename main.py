...
#                 transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])
    ...
    kth_dataset = kth.KTH(train=True, data_root='/data/dennis/data_uni', seq_len=20, image_size=64, transforms=trans)
    kth_train = DataLoader(dataset=kth_dataset, batch_size=BATCH_SIZE, shuffle=True, drop_last=True, num_workers=2, pin_memory=True)
    kth_test = kth.KTH(train=False, data_root='/data/dennis/data_uni', seq_len=20, image_size=64, transforms=trans)
...
#         'image_encoder': image_encoder().to(DEVICE)
        'encoder_c': drnet.Encoder(3, 128).to(DEVICE),
        'encoder_m': drnet.Encoder(6, 128, vae=True).to(DEVICE),
        'decoder': drnet.Decoder(128+128, 3).to(DEVICE),
...
#     optimizer_D = optim.RMSprop(model['D'].parameters(), lr=LR)
    ...
    # Training Procedure
    model, loss_record = train(model, kth_train, kth_test, optimizer_G, optimizer_G, N_EPOCHS, batch_size=BATCH_SIZE, epoch_size=EPOCH_SIZE, env_name=SAVED_PATH)
